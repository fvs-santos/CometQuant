...
describe('storage diagnostics', () => {
  ...
  it('reports estimates without requesting persistent storage', async () => {
    ...
    vi.stubGlobal('caches', {
      keys: vi.fn(async () => ['cometquant-shell-v23']),
      open: vi.fn(async () => ({
        match: vi.fn(async () => ({ text: async () => 'cometquant-shell-v23' }))
      }))
    })
    ...
    expect(report).toMatchObject({
      ...
      caches: { shell: ['cometquant-shell-v23'], shellReady: true, science: [], error: null },
