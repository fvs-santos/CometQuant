...
describe('storage diagnostics', () => {
  ...
  it('reports estimates without requesting persistent storage', async () => {
    ...
    vi.stubGlobal('caches', {
      keys: vi.fn(async () => ['cometquant-shell-v13']),
      open: vi.fn(async () => ({
        match: vi.fn(async () => ({ text: async () => 'cometquant-shell-v13' }))
      }))
    })
    ...
    expect(report).toMatchObject({
      ...
      caches: { shell: ['cometquant-shell-v13'], shellReady: true, science: [], error: null },
