...
function v4Analysis(overrides = {}) {
  const comparison = {
    ...
    ciLow: 5.835, ciHigh: 14.165, pRaw: 0.0026, pAdjusted: 0.0052, significant: true, direction: 'higher', increaseDetected: true
  }
  const blockAnova = {
  ...
  return {
    ...
    protocol: {
      ...
      alternative: 'two-sided', confidenceLevel: 0.95, includePrimaryReferenceAsZero: true,
      visualScoreDenominator: 'effective_counted_nucleoids', offTargetSlidesIncluded: true
    },
    population: {
    ...
    validation: {
      performed: true, independentExperimentCount: 3, minimumRequiredExperiments: 3, estimable: true,
      basalControlPresent: true, positiveControlPresent: true, viabilityDataAvailable: false,
      cellCompleteness: [], scoreOutOfRangeCount: 0,
      floorCeilingFlag: { flagged: false, ratio: 0.1, nearFloorCount: 0, nearCeilingCount: 0, threshold: 5 }
    },
    scores: { performed: true, population: 'primary_complete_blocks', cells: [] },
    blockAnova,
    primaryComparisons: {
      performed: true, family: 'each_primary_concentration_vs_reference', familySize: 1, comparisonMethod: 'dunnett',
      adjustment: 'dunnett', confidenceLevel: 0.95, confidenceIntervals: 'simultaneous', dunnettCriticalValue: 2.5,
      randomStateSeed: 20240601, omnibusGateUsed: false, comparisons: [comparison]
    },
    controlResponse: {
      performed: true, purpose: 'separate_validation_comparison', blockNumbers: [1, 2, 3], blockAnova,
      comparison: { ...comparison, pAdjusted: undefined, increaseDetected: undefined },
      notes: [{ code: 'low_residual_degrees_of_freedom', detail: 'Few residual degrees of freedom.' }]
    },
    trendAnalysis: {
    ...
    diagnostics: { performed: false, reason: { code: 'insufficient_blocks_for_influence_analysis', detail: 'Need at least four independent experiments.' } },
    interpretation: {
...
function reportScenario() {
  ...
  Object.assign(comparison, {
    ...
    referenceMean: 10, treatmentMean: 20, pRaw: 0.04, pAdjusted: 0.068, significant: false, increaseDetected: false
  })
  ...
  analysis.primaryComparisons.comparisons.push({
    ...comparison, treatmentIndex: 3, treatment: 'Compound 5 uM', treatmentMean: 30, difference: 20, pRaw: 0.0005, pAdjusted: 0.001, significant: true, increaseDetected: true
  })
  analysis.trendAnalysis.pageTrend.treatmentIndices = [0, 2, 3]
...
describe('safe exports', () => {
  it('uses the v3 transient explicit-selection schema in the analysis fixture', () => {
    ...
    expect(analysis.analysisSchemaVersion).toBe(4)
  ...
  it('exports slide corrections separately and safely in CSV and HTML', () => {
    ...
    const html = exporter.buildReportHtml(data, v4Analysis(), 'en')
  ...
  it('builds safe CSVs for every analysis v4 result', () => {
    const data = experiment({
    ...
    const outputs = {
      ...
      validation: exporter.buildValidationCsv(analysis),
      anova: exporter.buildBlockAnovaCsv(analysis),
      comparisons: exporter.buildPrimaryComparisonsCsv(analysis),
      control: exporter.buildControlResponseCsv(analysis),
      trend: exporter.buildTrendCsv(analysis),
      diagnosticsResiduals: exporter.buildDiagnosticsResidualsCsv(analysis),
      diagnosticsInfluence: exporter.buildDiagnosticsInfluenceCsv(analysis),
      interpretation: exporter.buildInterpretationCsv(analysis),
    ...
    expect(outputs.validation).toContain('"independent_experiment_count"')
    expect(outputs.anova).toContain('"treatment"')
    expect(outputs.comparisons).toContain('"p_adjusted"')
    expect(outputs.comparisons).toContain('"comparison_method"')
    expect(outputs.comparisons).toContain('"\'=DANGEROUS()"')
    expect(outputs.control).toContain('"p_raw"')
    expect(outputs.control).toContain('"notes"')
    expect(outputs.trend).toContain('"p_exact"')
    expect(outputs.trend).toContain('"direction_source"')
    expect(outputs.diagnosticsResiduals).toContain('"reason_code"')
    expect(outputs.diagnosticsInfluence).toContain('"reason_code"')
    expect(outputs.interpretation).toContain('"conclusion_code"')
    expect(outputs.design).toContain('"\'=DANGEROUS()"')
  })

  it('renders the human v4 report and escapes statistical labels', () => {
    const data = experiment()
    const analysis = v4Analysis()
    ...
    expect(html).toContain('Planned primary comparisons (Dunnett)')
    expect(html).toContain('Positive control response (paired t-test)')
    expect(html).toContain('Dose trend (Page L)')
    expect(html).toContain('Diagnostics and robustness')
    expect(html).toContain('Statistical methods')
    expect(html).toContain('Differences with simultaneous 95% CI')
    ...
    expect(html).not.toContain('Friedman')
    expect(html).not.toContain('arcsin')
    expect(html).toContain('&lt;img src=x onerror=alert(1)&gt;')
    expect(html).not.toContain('<img src=x onerror=alert(1)>')
  })
  ...
  it('puts a plain-language conclusion and individual replicate chart before technical details', () => {
    ...
    expect(html).toContain('Comparações contra a referência')
    expect(html).toContain('Tendência relacionada à concentração')
    expect(html).toContain('O controle positivo respondeu na direção esperada')
    expect(html).toContain('Evidência de aumento do dano ao DNA acompanhada de resposta ordenada')
    expect(html).toContain('mostraram aumento do dano ao DNA detectado(a) (Compound 5 uM)')
    expect(html).toContain('sem efeito detectado: Compound 1 uM')
    ...
    expect(html).toContain('* p ajustado por Dunnett &lt; 0,05')
    expect(html).not.toContain('Holm')
    expect(html).not.toContain('Friedman')
    expect(html).not.toContain('arcsin')
    expect(html).not.toContain('não-genotóxic')
  ...
  it('adds navigable traceability and a Dunnett-annotated primary column chart', () => {
    ...
    expect(columnChart.querySelectorAll('.dunnett-marker')).toHaveLength(1)
    ...
    expect(columnChart.querySelector('desc').textContent).toContain('p ajustado por Dunnett menor que 0,05')
  ...
  it('preserves the engine threshold and does not present zero SD as estimable with n=1', () => {
    ...
    analysis.primaryComparisons.comparisons[1].increaseDetected = false
    ...
    expect(chart.querySelectorAll('.dunnett-marker')).toHaveLength(0)
  ...
  it('adapts effect direction and wording for antigenotoxicity', () => {
    ...
    analysis.primaryComparisons.comparisons = analysis.primaryComparisons.comparisons.map((comparison, index) => ({
      ...
      treatmentMean: index === 0 ? 20 : 10, difference: index === 0 ? -28 : -38, direction: 'lower', significant: true, increaseDetected: true
    }))
    analysis.trendAnalysis.pageTrend.direction = 'decreasing'
    analysis.interpretation.conclusionCode = 'increase_detected_with_ordered_trend'
    analysis.descriptive.treatments = [
    ...
    const html = exporter.buildReportHtml(data, analysis, 'pt')
    expect(html).toContain('redução do dano ao DNA')
    expect(html).toContain('Redução detectada')
    const document = new DOMParser().parseFromString(html, 'text/html')
  ...
  it('provides content suitable for the complete ZIP', async () => {
    ...
    zip.file('data/validation.csv', exporter.buildValidationCsv(analysis))
    zip.file('data/block_anova.csv', exporter.buildBlockAnovaCsv(analysis))
    zip.file('data/primary_comparisons.csv', exporter.buildPrimaryComparisonsCsv(analysis))
    zip.file('data/control_response.csv', exporter.buildControlResponseCsv(analysis))
    zip.file('data/trend.csv', exporter.buildTrendCsv(analysis))
    zip.file('data/diagnostics_residuals.csv', exporter.buildDiagnosticsResidualsCsv(analysis))
    zip.file('data/diagnostics_influence.csv', exporter.buildDiagnosticsInfluenceCsv(analysis))
    zip.file('data/interpretation.csv', exporter.buildInterpretationCsv(analysis))
    ...
    expect(Object.keys(opened.files)).toEqual(expect.arrayContaining([
      ...
      'data/population.csv', 'data/validation.csv', 'data/block_anova.csv', 'data/primary_comparisons.csv', 'data/control_response.csv',
      'data/trend.csv', 'data/diagnostics_residuals.csv', 'data/diagnostics_influence.csv', 'data/interpretation.csv', 'data/study_design.csv',
